...
class TodosFilterFragment: BaseFragment() {
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        viewModel.observeCategories().apply {
            this.observe(this@TodosFilterFragment, Observer { processCategoryListReponse(it) })
        }
    }

    private fun processCategoryListReponse(categoriesFilterResult: Result<List<CategoryFilter>>?) {
        if (!isAdded || categoriesFilterResult === null) {
            return
        }
        if (categoriesFilterResult == Result.Loading) {
            return
        }
