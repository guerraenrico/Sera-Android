...
class AddCategoryFragmnet: BaseFragment() {
    ...
    private fun observeCreateCategory() {
        viewModel.observeCreateCategory().observe(this, Observer {
            result ->
            ...
            if (result is Result.Error) {
                Snackbar.make( root, result.exception.message ?: "An error accour while creating the category", Snackbar.LENGTH_LONG).show()
            }
        })
    }
}